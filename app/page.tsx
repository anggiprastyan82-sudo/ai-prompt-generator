...
import OfficeScene, { SceneAgent } from "./office-scene";
...
const toSceneAgent = (agent: Agent): SceneAgent => ({ name: agent.name, status: agent.status, icon: agent.icon, color: agent.color });
...
export default function Home() {
  ...
  return <main className="office-app">
    ...
    <section className="app-content"><header className="topbar"><button className="mobile-menu" onClick={() => setMenuOpen(true)}>☰</button><div><small>VIRTUAL OPERATIONS / LIVE</small><h1>{page}</h1></div><div className="astra"><span>AI</span><p><b>Astra Command</b><small>Semua agent berjalan sesuai rencana.</small></p></div><div className="top-avatar">OP</div></header>
      ...
      {page === "AI Office" && <><section className="office-layout"><div className="office-main"><div className="office-heading"><div><span>LIVE VIRTUAL OFFICE / 3D ISOMETRIC VIEW</span><h2>MAINku Operations Floor</h2></div><p><i /> 6 agent online</p></div><OfficeScene agents={agents.map(toSceneAgent)} activeAgent={toSceneAgent(activeAgent)} onSelect={(sceneAgent) => { const selected = agents.find((agent) => agent.name === sceneAgent.name); if (selected) setActiveAgent(selected); }} /><div className="office-hint">Drag untuk menggeser kamera, scroll untuk zoom, dan klik employee untuk melihat detail.</div></div>
